...
TEST_CASE("unit_hash_smart_ptr") {
    ...
    check(std::make_unique<uint64_t>());
    check(std::make_unique<uint64_t>(new uint64_t{123}));

    check(std::make_shared<uint64_t>());
    check(std::make_shared<uint64_t>(new uint64_t{123}));
#else
